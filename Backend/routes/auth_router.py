...
async def signup(user: UserCreate, db: AsyncSession = Depends(get_db)):
    try:
        ...
        query_cin = select(Patient).filter(Patient.cin == user.cin)
        result_cin = await db.execute(query_cin)
        db_user = result_cin.scalar_one_or_none()

        if db_user:
            raise HTTPException(status_code=400, detail="CIN is already registered")

        # Check if email already exists
        query_email = select(Patient).filter(Patient.email == user.email)
        ...
        return {"message": "Patient created successfully. Please check your email for verification."}
...
async def login(user: LoginRequest, db: AsyncSession = Depends(get_db)):
    try:
        # Find the user by email
        query = select(Patient).filter(Patient.email == user.email)
        result = await db.execute(query)
        db_user = result.scalar_one_or_none()
